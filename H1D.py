class H1D(object):
    ...
    def __init__(self, n, lo, hi):
        ...
        self._data     = [ (0.0, 0) for k in range(self._n)]
    ...
    def fill(self, x, weight = 1.0):
        ...
        if (idx < 0):
            t = (self._underflow[H1D.DATA] + weight, self._underflow[H1D.NOE] + 1)
            self._underflow = t
            return

        if (idx >= self._n):
            t = (self._overflow[H1D.DATA] + weight, self._overflow[H1D.NOE] + 1)
            self._overflow = t
            return

        t = (self._data[idx][H1D.DATA] + weight, self._data[idx][H1D.NOE] + 1)
        self._data[idx] = t
        return
